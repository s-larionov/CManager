...
class CManager_File_Image extends CManager_File {
	...
	public function load() {
		if (!$this->exists()) {
			throw new CManager_File_Exception("File {$this->getFilename()} doesn't exists.");
		}
		if (!$this->isReadable()) {
			throw new CManager_File_Exception("File {$this->getFilename()} doesn't readable.");
		}

		// determine image format
		list( , , $type) = getimagesize($this->getFilename());
		switch ($type) {
			case IMAGETYPE_JPEG:
			case IMAGETYPE_JPEG2000:
				$this->_imageHandle = imagecreatefromjpeg($this->getFilename());
				break;
			case IMAGETYPE_PNG:
				$this->_imageHandle = imagecreatefrompng($this->getFilename());
				break;
			case IMAGETYPE_GIF:
				$this->_imageHandle = imagecreatefromgif($this->getFilename());
				break;
			default:
	...
	public function getImageType() {
		if ($this->_imageType === null) {
			// determine image format
			list( , , $this->_imageType) = getimagesize($this->getFilename());
		}
...
//		throw new CManager_File_Image_Exception('Unsupported image type');
	...
	protected function _toPng($handle, $quality = self::DEFAULT_QUALITY_PNG) {
		if (!is_resource($handle)) {
			throw new CManager_File_Image_Exception('Resource is not valid');
		}
		ob_start();
	...
	public function resize($width = null, $height = null,
		...
		if (!is_resource($targetImage)) {
			throw new CManager_File_Image_Exception('Cannot initialize new GD image stream');
		}
		imagesavealpha($this->_imageHandle, true);

		// заливаем новую картинку, если она должна быть с полями
		if ($fields == self::RESIZE_WITH_FIELDS) {
			$colorRgb = $this->_rgbToArray($backgroundColor);
		...
		if (!$result) {
			throw new CManager_File_Image_Exception('Cannot resize image');
		}

		// Free a memory from the source image
		imagedestroy($sourceImage);

		$this->_imageHandle = $targetImage;
	 ...
	 * @author: Yetty
	 */
	protected function _rgbToArray($rgb) {
		return array(
	...
	public function getSliceHandle($imageWidth, $imageHeight,
		...
		if ($column >= $columnsCount || $column < 0) {
			throw new CManager_File_Image_Exception('Column must in range 0-' . ($columnsCount - 1));
		}
		if ($row >= $rowsCount || $row < 0) {
			throw new CManager_File_Image_Exception('Row must in range 0-' . ($rowsCount - 1));
		}
