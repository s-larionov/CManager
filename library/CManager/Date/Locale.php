...
class CManager_Date_Locale {
	...
	public function toString($format = null, $locale = null) {
		...
		if (strpos($format, self::DATE_MONTH_NAME) !== false) {
			...
			} catch (CManager_Date_Exception $e) {}
		}
	...
	/**
	 * @param string $locale
	 * @return CManager_Date_LocaleData
	 */
	public function getLocalizedData($locale) {
		if (!array_key_exists($locale, self::$_localeData)) {
			try {
				self::$_localeData[$locale] = new CManager_Date_LocaleData(__DIR__ . '/Locale/' . $locale . '.xml');
			} catch (CManager_Date_Exception $e) {
				self::$_localeData[$locale] = null;
				throw $e;
			}
		} elseif (self::$_localeData[$locale] === null) {
			throw new CManager_Date_Exception('File for current locale not found');
		}
