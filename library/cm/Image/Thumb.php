...
class cm_Image_Thumb {
	...
	public static function imageCreateFromFile($filename) {
		if (!is_file($filename) || !is_readable($filename)) {
			throw new cm_Image_Exception('Unable to open file "' . $filename . '"');
		}
		...
		throw new cm_Image_Exception('Unsupported image type');
	}
	...
	public static function imageCreateFromString($string) {
		if (!is_string($string) || empty($string)) {
			throw new cm_Image_Exception('Invalid image value in string');
		}
	...
	public static function output($input, $output = null, $options = array()) {
		...
		if ( ! $renderImage ) {
			throw new cm_Image_Exception('Error rendering image');
		}
		...
		if ( empty($output) ) {
			...
			if ( ! headers_sent() ) {
				...
				throw new cm_Image_Exception('Headers have already been sent. Could not display image.');
			}
		}
		...
		case IMAGETYPE_GIF:
			...
			break;
		default:
			throw new cm_Image_Exception('Image type ' . $content_type . ' not supported by PHP');
		}

		// Output image (to browser or to file)
		if ( ! $result ) {
			throw new cm_Image_Exception('Error output image');
		}
	...
	public static function render($input, $options = array()) {
		...
		if ( ! is_resource($sourceImage) ) {
			throw new cm_Image_Exception('Invalid image resource');
		}
		...
		if ( ! is_resource($targetImage) ) {
			throw new cm_Image_Exception('Cannot initialize new GD image stream');
		}
		...
		if (!$result) {
			throw new cm_Image_Exception('Cannot resize image');
		}
