...
abstract class cm_CacheStorage_Abstract {
	...
	final public static function factory($storage) {
		if (!isset(self::$instance[$storage])) {
			try {
				$object = new ReflectionClass('cm_CacheStorage_'. $storage);
				self::$instance[$storage] = $object->newInstance();
			} catch (ReflectionException $e) {
				throw new cm_CacheStorage_Exception('trying_use_undefined_cache_strorage', $storage);
			}
		}
