...
class cm_Collection implements cm_Collection_Interface {
	...
	public function offsetSet($offset, $value) {
		if (is_array($value)) {
			$class = strpos(strtolower(get_class($this)), 'cm_config') === 0 ? 'cm_Config' : get_class($this);
	...
	public function offsetGet($offset, $throw = false) {
		if (!isset($this->_collectionData[$offset])) {
			if ($throw) {
				throw new cm_Collection_Exception("Undefined offset $offset.");
			}
			return null;
		}
